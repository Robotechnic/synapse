...
#notion("synapse", "Synapse", "synapses", "Synapses")
#notion("nervous system", url:"https://en.wikipedia.org/wiki/Nervous_system")
#notion("neuron", "neurons", url:"https://en.wikipedia.org/wiki/Neuron")
#notion("signal", "signals", url:"https://en.wikipedia.org/wiki/Cell_signaling")

#synapse-config(mode: "composition")

#show: quote-rule

In the "nervous system", a ""synapse"" is a structure that allows a "neuron" to exchange (receive or send) "signals" with another cell in its immediate vicinity. "Synapses" can be classified as either chemical or electrical, depending on the mechanism of "signal" transmission between "neurons". In the case of electrical "synapses", "neurons" are coupled bidirectionally with each other through gap junctions and have a connected cytoplasmic milieu. These types of "synapses" are known to produce synchronous network activity in the brain, but can also result in complicated, chaotic network level dynamics. Therefore, "signal" directionality cannot always be defined across electrical synapses.
